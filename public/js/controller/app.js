...
angular.module('laborGate', ['ngRoute', '720kb.datepicker'])
	...
	.otherwise('/404', { 
		template: "<h3 style='text-align: center'>Sorry, we can't find that.</h3>"
	})
})
...
.controller('appController', ($scope, $http, $rootScope) => { 
	
	// Creation of VM to prevent safeguards.
	var vm = this;
    ...
    $rootScope.assignTo = ["Andrew", "Jackie", "Karl"];
...
*/
	...
	$scope.authenticateUser = () => { 
		console.log("Scope Info:" + $scope.formData.email);
		...
		.success((data) => { 
			...
			if(data.length === 1) alert("Welcome user: " + data[0].user_name);
	...
	$scope.createTestUser = () => {
		console.log($scope.formData);
		$http.post('/api/usertest/', $scope.formData)
		.success((data) => { 
			$scope.formData = {};
			$scope.userData = data;
			console.log(data);
